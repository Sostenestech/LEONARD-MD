...
zokou({ nomCom: "𝗟𝗲𝗼𝗻𝗮𝗿𝗱 𝗠𝗗",
    ...
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        repondre("* processing...*");
        await zk.sendMessage(origineMessage, { image: { url:anu.image}, caption: "* \t *𝑳𝒐𝒈𝒐 𝑩𝒚 𝗟𝗲𝗼𝗻𝗮𝗿𝗱 𝗠𝗗*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "dragonball", categorie: "Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    ...
    try {
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: "* \t *𝑳𝒐𝒈𝒐 𝑩𝒚 𝗟𝗲𝗼𝗻𝗮𝗿𝗱 𝗠𝗗*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "naruto", categorie: "Logo", reaction: "⛩" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t\t *𝑳𝒐𝒈𝒐 𝑩𝒚 𝗟𝗲𝗼𝗻𝗮𝗿𝗱 𝗠𝗗*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
var {ms,repondre,arg,prefixe}=commandeOptions;
  ...
  try{
    ...
    var img =await mumaker.ephoto(lien,arg);
    repondre(" processing ...")
    await zk.sendMessage(dest,{image:{url:img.image},caption:"  *Logo by 𝗟𝗲𝗼𝗻𝗮𝗿𝗱 𝗠𝗗*"})
  }catch(e){repondre(`🥵🥵 ${e}`)}
...
zokou({nomCom:"gold",categorie:"Logo",reaction:"🧚🏿‍♀️"},async(dest,zk,commandeOptions)=>{
  ...
  try{
    ...
    var img = await mumaker.ephoto(lien,arg);
   repondre("processing ...")
    await zk.sendMessage(dest,{image:{url:img.image},caption:" *𝑮𝒆𝒏𝒆𝒓𝒂𝒕𝒆𝒅 𝒃𝒚 𝗟𝗲𝗼𝗻𝗮𝗿𝗱 𝗠𝗗*"},{quoted:ms})
  }catch(e){repondre(e)}
})
...
zokou({ nomCom: "gif×1", categorie: "Logo", reaction: "😋" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    if (!arg || arg === "") {
      repondre(`${prefixe}gif×1 𝗟𝗲𝗼𝗻𝗮𝗿𝗱 𝗠𝗗`);
      return;
    }
